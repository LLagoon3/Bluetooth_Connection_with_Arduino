...
int p_count(int current_val){
  if(current_val - PREV_VALUE > 10){ // UP
    COUNT_FLAG = true;
    MAX_VALUE = current_val;
  }
  else if((current_val - PREV_VALUE < -10) && (COUNT_FLAG == true)){ // DOWN
    COUNT_FLAG = false;
    MAX_VALUE = 0;
    COUNT++;
  }
...
void loop() {
  ...
  int count = p_count(fsrVals.tmpSensorValueList[0]);
  // double bpm;
  // int tmpArray[size];
  ...
  // Serial.println("res : " + String(fsrVals.sensorValue0) + " " + String(fsrVals.sensorValue1));
  // Serial.print("tmp val : " );
  // printArray(fsrVals.tmpSensorValueList);
  // Serial.println("orginal val : " + String(fsrVals.sensorValueList[4]) + " " + String(fsrVals.sensorValueList[3]));
...
//String(int(bpm)
  ...
  String data = String(cycle * sendInterval) + '_' + fsrVals.sensorValueString + "_" + String(fsrVals.sensorValue0) + "," + String(fsrVals.sensorValue1) + "_" + String(count) + "_" + String(int(bpm(count, currentTime)));
  dataSend(data);
  Serial.println(data);
